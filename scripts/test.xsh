...
def cleanup_ci_lake_build(lampe_dir):
    # Per-test .lake/build is the cache key target now; do not wipe it
    # between tests on CI. Keep the helper around so callers do not break.
    return
...
def run_test_in_dir(working_dir, original_dir, update_mode):
    ...
    if not update_mode:
        assert_extraction_matches(working_dir)
...
def run_test(dir_path, update_mode):
    # We always run the test in its checked-in directory now. Lake's
    # incremental builder uses .lake/build under that dir as the cache,
    # so leaving artifacts in place between local + CI runs is the whole
    # point. Reproducibility is asserted via `git diff` instead of a
    # tempdir comparison.
